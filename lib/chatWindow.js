...
export const ChatWindow = GObject.registerClass({
    ...
    _buildHeader(parent) {
        ...
        const titleText = (this._settings.get_string('header-text') || 'GNOME LLM') + (version ? ` v${version}` : '');
...
The AI model is now active and ready to respond to your questions. Start typing below to begin our conversation!`;
    ...
    async _loadModels() {
        try {
            ...
            this._availableModels = models.filter(Boolean);

            if (this._availableModels.length > 0) {
                let currentModel = this._settings.get_string('current-model');
                if (!currentModel || !this._availableModels.includes(currentModel)) {
                    currentModel = this._availableModels[0];
                    this._settings.set_string('current-model', currentModel);
                }
                
                this._modelSelector.set_label(currentModel);
                this._modelSelector.set_reactive(true);
                log(`Loaded ${this._availableModels.length} models, current: ${currentModel}`);
                
                // Add model loaded confirmation message, but only if the chat is empty
                // (i.e., only contains the bottom spacer).
                if (this._chatBox.get_n_children() <= 1) {
                    this._addModelLoadedMessage(currentModel, this._availableModels.length);
                }
            } else {
...
Please ensure:
1.  Ollama is installed. If not, download it from [https://ollama.ai](https://ollama.ai).
2.  The Ollama application is running on your computer.
3.  The server URL in the extension settings (⚙️ icon) is correct (default is http://localhost:11434).
4.  Your system has network connectivity.

You can try to refresh the model list via the model selector dropdown once Ollama is running.`);
            }
        }
    }
    
    _showModelSelector() {
        if (this._modelSelectorMenu && this._modelSelectorMenu.isOpen) {
            this._modelSelectorMenu.close();
            return;
        }

        this._modelSelectorMenu = new PopupMenu.PopupMenu(this._modelSelector, 0.5, St.Side.TOP);
        this._modelSelectorMenu.actor.add_style_class_name('gnomellm-model-selector-menu');
        this.contentLayout.add_child(this._modelSelectorMenu.actor);

        this._modelSelectorMenu.connect('open-state-changed', (menu, isOpen) => {
            if (!isOpen) {
                this._modelSelectorMenu.destroy();
                this._modelSelectorMenu = null;
            }
        });

        this._modelSelectorMenu.removeAll();

        if (this._availableModels.length === 0) {
            this._modelSelectorMenu.addMenuItem(new PopupMenu.PopupMenuItem('No models found', { reactive: false }));
        } else {
            const currentModel = this._settings.get_string('current-model');
            for (const modelName of this._availableModels) {
                const menuItem = new PopupMenu.PopupMenuItem(modelName);
                if (modelName === currentModel) {
                    menuItem.setOrnament(PopupMenu.Ornament.DOT);
                }
                menuItem.connect('activate', () => {
                    this._selectModel(modelName);
                    if (this._modelSelectorMenu) {
                        this._modelSelectorMenu.close();
                    }
                });
                this._modelSelectorMenu.addMenuItem(menuItem);
            }
        }

        if (this._availableModels.length > 0) {
            this._modelSelectorMenu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
        }

        const refreshItem = new PopupMenu.PopupMenuItem('Refresh Models');
        ...
        this._modelSelectorMenu.addMenuItem(refreshItem);

        this._modelSelectorMenu.open(true);
    }
    ...
    _selectModel(modelName) {
        ...
        this._settings.set_string('current-model', modelName);
        this._modelSelector.set_label(modelName);

        // Start a new chat, providing a specific message for the model switch
        const initialMessage = `✅ **Model Switched**

Now using **${modelName}**. Your conversation has been cleared.`;
        this._startNewChat(initialMessage);
    }
...
-   The Ollama application might not be running. Please start it.
-   If Ollama is not installed, you can download it from [https://ollama.ai](https://ollama.ai).
-   The selected model ('${this._settings.get_string('current-model')}') might not be available or fully pulled.
-   There might be network connection issues to the Ollama server.

Please check your Ollama setup and try again. You can also try selecting a different model or refreshing the model list.`;
            ...
            const errorMessage = `**Error:** Failed to send message to Ollama. 

**Details:** ${e.message}

Please ensure:
1.  Ollama is installed. If not, get it from [https://ollama.ai](https://ollama.ai).
2.  The Ollama application is running.
3.  The server URL in extension settings is correct.
4.  At least one model is available and selected.

You can test the connection and refresh models in the extension settings (⚙️ icon).`;
    ...
    _setInProgress(inProgress) {
        ...
        if (inProgress) {
            ...
            this._sendButton.child.icon_name = 'document-send-symbolic';
    ...
    _startNewChat(initialMessage = null) {
        ...
        try {
            ...
            if (initialMessage) {
                this._addMessage('assistant', initialMessage);
            } else {
                this._addWelcomeMessage();
            }
