...
Click the **⚙️** settings button to configure the connection.`);
    ...
    _scrollToBottom() {
        ...
        // Use idle_add to wait until the UI has processed updates.
        GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
            try {
                const vadjustment = this._chatView.vadjustment;
                if (vadjustment) {
                    const bottom = vadjustment.get_upper() - vadjustment.get_page_size();
                    // Only scroll if we are not already at the bottom.
                    if (bottom > vadjustment.get_value()) {
                        // Animate scrolling to the new bottom using the adjustment
                        vadjustment.ease(vadjustment.get_value(), bottom, {
            ...
            return GLib.SOURCE_REMOVE; // Ensure the callback runs only once
        });
    }
