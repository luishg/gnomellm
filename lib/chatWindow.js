...
const SCROLL_ANIMATION_TIME = 250; // ms
...
export const ChatWindow = GObject.registerClass({
    ...
    _init(extension) {
        ...
        try {
            ...
            // Defer model loading to after window opens
    ...
    _buildHeader(parent) {
        ...
        this._header.add_child(this._modelSelector);
    ...
    _buildChatHistory(parent) {
        log('Building chat history view...');
        ...
        try {
            ...
            parent.add_child(this._chatView);
            ...
            log('Chat history view built successfully');
            
            // DON'T add welcome message during initialization
            // We'll add it after the window opens and everything is ready
...
Current model: **${this._settings.get_string('current-model')}**

Start chatting by typing a message below. You can use:
- \`inline code\` for code snippets
- **bold text** for emphasis
- *italic text* for style

Press **Escape** to close this window or use the hotkey **${this._settings.get_strv('chat-hotkey')[0] || 'Super+Shift+Space'}** to toggle it.`;
    ...
    async _loadModels() {
        try {
            ...
            if (models.length > 0) {
                ...
                this._modelSelector.set_label(currentModel || models[0]);
    ...
    _addMessage(role, text) {
        ...
        try {
            ...
            if (role === 'assistant') {
                ...
                messageLabel = new St.Label({ text: text });
    ...
    _scrollToBottom() {
        // Defer scrolling if the window isn't ready yet
        if (!this.get_parent() || !this._chatView || !this._isUIBuilt) {
            log('Deferring scroll - window not ready');
            return;
        }
        
        try {
            const vscroll = this._chatView.get_vscroll_bar();
            if (vscroll && vscroll.adjustment) {
                const v_adj = vscroll.adjustment;
                v_adj.ease(v_adj.upper, {
                    duration: SCROLL_ANIMATION_TIME,
                    mode: Clutter.AnimationMode.EASE_OUT_QUAD,
                });
                log('Scrolled to bottom successfully');
            } else {
                log('Scrollbar not available yet');
            }
        } catch (error) {
    ...
    _updateAssistantMessage(newText) {
        ...
        // The first child should be the label added by _addMessage
        let label = container.get_first_child();

        if (label) {
            // Update existing label
            label.destroy();
            label = this._markdownRenderer.render(newText);
            label.style_class = 'gnomellm-message-text gnomellm-assistant-message-text';
...
You can test the connection in the extension settings.`;
    ...
    _onOpened() {
        ...
        GLib.timeout_add(GLib.PRIORITY_DEFAULT, 200, () => {
            ...
            // Give focus to the entry field and scroll to bottom
            this._entry.grab_key_focus();
            this._scrollToBottom();
